trigger CampaignTrigger on Campaign (after insert, before insert, after undelete, before delete) {
    if(trigger.isinsert && trigger.Isbefore){
        CampaignTriggerHandler.CampaignTriggerBeforeInsert(trigger.new);        
    }
    if(trigger.isInsert && trigger.isAfter)
            {   
	//		CampaignTriggerHandler.CampaignTriggerAfterInsert(trigger.new);
            }
    ...
    if ( trigger.isdelete ){
        ...
        deletePermantently_apIds = [ select Id, Name , LastModifiedDate from ActionPlan__c where Campaign__c in : cIds and isDeleted = true ALL ROWS ];
        ...
        for( Campaign a : [Select (Select Id From Action_Plans__r) From Campaign a where Id in : cIds]){
    ...
    if ( trigger.isUnDelete ){
        ...
        list <ActionPlan__c> aPs = [ select Id from ActionPlan__c where Campaign__c in : cIds ALL ROWS ];
