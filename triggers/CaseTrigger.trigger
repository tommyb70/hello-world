trigger CaseTrigger on Case (before delete, before insert, before update, after update, after delete, after insert, after undelete) {

    if (trigger.isAfter && !trigger.IsDelete) {
        CaseTriggerHandler.runCaseSharing(trigger.new, trigger.oldMap);
        CaseTriggerHandler.runCaseAssignmentRules(trigger.new);
    }
    
    if (trigger.isBefore && !trigger.IsDelete) {
        CaseTriggerHandler.setFieldValues(trigger.new, trigger.oldMap);
    }    
    
    //if(trigger.isInsert && trigger.isafter) {
    //    CaseTriggerHandler.CaseTriggerAfterInsert(trigger.new);                
    //}

    //if(trigger.isUpdate && trigger.isafter) {
    //    CaseTriggerHandler.CaseTriggerAfterUpdate(trigger.new);                
    //}
